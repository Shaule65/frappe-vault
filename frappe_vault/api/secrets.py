...
@frappe.whitelist()
def list(search=None, secret_type=None, folder=None, favorites_only=False, tag=None, limit=20, offset=0, order_by="modified desc"):
    from frappe_vault.services.secret_service import get_secrets
    return get_secrets(search=search, secret_type=secret_type, folder=folder, favorites_only=frappe.utils.cint(favorites_only), tag=tag, limit=int(limit), offset=int(offset), order_by=order_by)
